...
struct ControlFlowLiveness{
	...
	std::string toDot(YulNameLabelRegistry const& _labels) const;
};
...
struct ControlFlow
{
	...
	std::string toDot(YulNameLabelRegistry const& _labels, ControlFlowLiveness const* _liveness=nullptr) const
	{
		if (_liveness)
