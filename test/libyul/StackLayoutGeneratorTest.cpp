...
{
public:
	explicit VariableSlotToStringTransform(YulNameLabelRegistry const& _labels): m_labels(_labels) {}
...
{
public:
	StackLayoutPrinter(std::ostream& _stream, StackLayout const& _stackLayout, Dialect const& _dialect, YulNameLabelRegistry const& _labels):
...
TestCase::TestResult StackLayoutGeneratorTest::run(std::ostream& _stream, std::string const& _linePrefix, bool const _formatted)
{
	...
	YulNameDispenser nameDispenser(yulStack.parserResult()->code()->labels());
