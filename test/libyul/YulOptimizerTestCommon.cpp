	...
	m_optimizedObject(std::make_shared<Object>(*_obj))
{
	...
	m_labels = std::make_unique<YulNameLabelRegistry>(m_object->code()->labels());
	m_nameDispenser = std::make_unique<YulNameDispenser>(*m_labels);
	m_namedSteps = {
		{"disambiguator", [&]() { return disambiguate(); }},
		{"nameDisplacer", [&]() {
			auto block = disambiguate();
			updateContext();
			YulNameLabelRegistryBuilder builder{m_nameDispenser->generateNewLabels(block, m_context->dialect)};
			...
			m_labels = std::make_unique<YulNameLabelRegistry>(builder.build());
			m_nameDispenser = std::make_unique<YulNameDispenser>(*m_labels, labelsToFree);
			NameDisplacer{
				*m_nameDispenser,
				namesToFree
			}(block);
		...
		{"stackCompressor", [&]() {
			...
			{
				m_labels = std::make_unique<YulNameLabelRegistry>(m_nameDispenser->generateNewLabels(block, m_context->dialect));
				m_nameDispenser = std::make_unique<YulNameDispenser>(*m_labels);
		...
		{"fullSuite", [&]() {
			...
			m_nameDispenser = std::make_unique<YulNameDispenser>(m_optimizedObject->code()->labels());
		...
		{"fakeStackLimitEvader", [&]()
		{
			...
			{
				explicit FakeUnreachableGenerator(YulNameLabelRegistry const& _labels): m_labels(_labels) {}
				...
				{
					if (!YulNameLabelRegistry::empty(_var) && m_labels(_var).front() == '$')
				...
				YulName m_currentFunction = YulNameLabelRegistry::emptyName();
